...
function saveRecipesToStorage(recipes) {
	...
	let recipeArray = existingRecipes ? JSON.parse(existingRecipes) : [];
	recipes.forEach((recipe) => {
		recipeArray.push(recipe);
	});
	const updatedRecipes = JSON.stringify(recipeArray);
...
function initFormHandler() {
	...
	submitButton.addEventListener('click', () => {
		...
		const existingRecipes = localStorage.getItem('recipes');
		const dataArray = existingRecipes ? JSON.parse(existingData) : [];
		dataArray.push(recipeObject);
		const updatedData = JSON.stringify(dataArray);
		localStorage.setItem('recipes', updatedData);
	});
	...
	clearStorageButton.addEventListener('click', () => {
		...
		let recipeElements = document.querySelectorAll('recipe-card');
